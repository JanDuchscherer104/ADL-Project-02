...
from ..global_configs.config import _AbstractExperimentConfig
from ..utils import CONSOLE, BaseConfig
from .lit_datamodule import LitNosmodDatamodule
from .lit_module import LitNOSModModule


class ExperimentConfig(BaseConfig["TrainerFactory"], _AbstractExperimentConfig):
    target: Type["TrainerFactory"] = Field(default_factory=lambda: TrainerFactory)

    # def setup_target(self, **kwargs: Any) -> "TrainerFactory":
    #     return self.target(self, **kwargs)


class TrainerFactory:
    def __init__(self, config: _AbstractExperimentConfig):
    ...
    def setup_target(
        cls,
        config: _AbstractExperimentConfig,
    ) -> Trainer:
        ...
        **trainer_kwargs,
        ...
        if isinstance(config.from_ckpt, Path):
            ...
            lit_module = LitNOSModModule.load_from_checkpoint(
                config.from_ckpt, hparams=config.module_config
            )
        else:
            lit_module = config.module_config.setup_target()
        lit_datamodule = config.datamodule_config.setup_target()
